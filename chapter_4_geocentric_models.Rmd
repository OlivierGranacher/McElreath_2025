...
ggplot(d_new, aes(x = weights)) +
  geom_ribbon(aes(ymin = q_1, ymax = q_99), alpha = .5) + 
...
```{r pred_ind_values}
weights_new
# fonction de génération des valeurs de prédiction individuelles
pred_height <- function(mu, sigma) {
  rnorm(length(mu), mu, sigma)
}
# post long : post values for each new weight
grid <- expand.grid(post, weights_new)
```
