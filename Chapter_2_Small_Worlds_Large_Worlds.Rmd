...
ggplot(results, aes(p, post)) +
  geom_line() +
  labs(title = glue("Posterior for W = {W} and L = {L} - Grid Method with {N} points"))
```
...
for (i in 2:N) {
  ...
  # p_new <- runif(1) converge moins bien :)
...
```{r plot_result, include=TRUE}
# Plot of chain
ggplot(data.frame(x = 1:N, p = p), aes(x, p)) + 
  geom_line() +
  geom_hline(yintercept = p_moy, color = "darkorange") +
  labs(title = glue("Posterior for W = {W} and L = {L} - MCMC Method with {N} points"))
# Plot of posterior
ggplot(data.frame(x = 1:N, p = p), aes(p)) +
  geom_density() + 
  geom_vline(xintercept = p_moy, color = "darkorange") +
  labs(title = glue("Posterior for W = {W} and L = {L} - MCMC Method with {N} points"))
```
