...
d_tot <- d
d <- d[d$age>18, ]
plot(d$height ~ d$weight, main = "Corrélation weight / height for age > 18")
plot(d_tot$height ~ d_tot$weight, main = "Corrélation weight / height for all ages")
...
## Création de la table des valeurs moyennes
...
weights_new <- seq(20, 70, 1)
values <- f_height_vec(weights_new)
q_1 <- apply(values, 2, FUN = quantile, 0.01)
q_50 <- apply(values, 2, FUN = quantile, 0.5)
q_99 <- apply(values, 2, FUN = quantile, 0.99)
d_new <- data.frame(
  weights = weights_new,
  q_1,
  q_50,
...
## Plot ic height
ggplot(d_new, aes(x = weights)) +
  geom_ribbon(aes(ymin = q_1, ymax = q_99), alpha = .5, fill = "pink") + 
  geom_point(data = d, aes(weight, height), alpha = .5) +
  labs(title = "IC 98 prediction Mean due à l'incertitude sur les coefficients")
  
```

## Visualisation de la prediction sur les valeurs individuelles

```{r pred_ind_values}
weights_new
# fonction de génération des valeurs de prédiction individuelles avec sigma
b_mean <- mean(post$b_weight)
a_mean <- mean(post$b_Intercept)
s_mean <- mean(post$sigma)
# function de prediction de valeur individuelle
pred_height_ind <- function(w, a = a_mean, b = b_mean, s = s_mean) {
  rnorm(length(a), a + b * w, s)
}
pred_height_ind(50)
pred_height_ind(weights_new)
pred_height_ind(50, post$b_Intercept, post$b_weight, post$sigma)
# Matrice de prediction des valeurs individuelles de w pour chaque ligne du postérieur
mat_result <- sapply(weights_new, FUN = pred_height_ind, a = post$b_Intercept, b = post$b_weight, s = post$sigma)
str(mat_result)
```

```{r plot_ic_ind, include=TRUE}
ic_ind <- apply(mat_result, MARGIN = 2, FUN = quantile, probs = c(0.05, 0.95) )
ic_ind <- cbind(weights_new, as.data.frame(t(ic_ind)))
ggplot(ic_ind, aes(x = weights_new)) +
  geom_line(aes(y = `5%`), lty = 3) +
  geom_line(aes(y = `95%`), lty = 3) + 
  labs(title = "IC prediction valeurs individuelles")
```
